...
import { View, Text, StyleSheet, Alert, ToastAndroid, Platform, TouchableOpacity, SafeAreaView } from 'react-native';
...
const generatePriceData = (startPrice: number, trend: string, volatility: number): number[] => {
  ...
  const trendFactor = 
    trend === 'up' ? 0.005 : 
    trend === 'down' ? -0.005 : 
    trend === 'volatile' ? 0 : 0.001; // stable has slight upward bias
...
const StockCard: React.FC<StockCardProps> = ({ item, onBuy, onSkip }) => {
  ...
  const chartData = {
    ...
    datasets: [{
      ...
      color: (opacity = 1) => item.priceHistory[0] < item.priceHistory[item.priceHistory.length-1] 
  ...
  const priceChange = item.priceHistory[item.priceHistory.length-1] - item.priceHistory[0];
    ...
    <View style={styles.card}>
      ...
      <View style={[
        ...
        <Text style={styles.analysisText}>
          {item.trend === 'up' ? 'Showing strong upward momentum.' : 
           item.trend === 'down' ? 'Currently in a downward trend.' : 
           item.trend === 'volatile' ? 'Experiencing high volatility.' : 
...
const App = () => {
  // Generate initial stock data with price histories
  const generateInitialStocks = (): StockItem[] => {
    const stockData = [
      { id: '1', name: 'Tata Consultancy', ticker: 'TCS', basePrice: 3400, trend: 'up', volatility: 0.3 },
      { id: '2', name: 'Infosys Ltd', ticker: 'INFY', basePrice: 1600, trend: 'stable', volatility: 0.2 },
      { id: '3', name: 'Reliance Industries', ticker: 'RELIANCE', basePrice: 2700, trend: 'up', volatility: 0.4 },
      { id: '4', name: 'HDFC Bank', ticker: 'HDFCBANK', basePrice: 1500, trend: 'stable', volatility: 0.2 },
      { id: '5', name: 'Zomato', ticker: 'ZOMATO', basePrice: 120, trend: 'volatile', volatility: 0.7 },
      { id: '6', name: 'Wipro Ltd', ticker: 'WIPRO', basePrice: 400, trend: 'down', volatility: 0.3 },
      { id: '7', name: 'Tata Motors', ticker: 'TATAMOTORS', basePrice: 650, trend: 'up', volatility: 0.5 },
      { id: '8', name: 'ICICI Bank', ticker: 'ICICIBANK', basePrice: 950, trend: 'stable', volatility: 0.2 },
      { id: '9', name: 'Adani Enterprises', ticker: 'ADANIENT', basePrice: 2200, trend: 'volatile', volatility: 0.8 },
      { id: '10', name: 'Asian Paints', ticker: 'ASIANPAINT', basePrice: 3100, trend: 'down', volatility: 0.3 },
      { id: '11', name: 'Bajaj Finance', ticker: 'BAJFINANCE', basePrice: 6800, trend: 'volatile', volatility: 0.6 },
      { id: '12', name: 'Bharti Airtel', ticker: 'BHARTIARTL', basePrice: 870, trend: 'up', volatility: 0.4 },
    ];

    return stockData.map(stock => {
      const priceHistory = generatePriceData(stock.basePrice, stock.trend, stock.volatility);
      return {
        id: stock.id,
        name: stock.name,
        ticker: stock.ticker,
        ...
        trend: stock.trend as 'up' | 'down' | 'volatile' | 'stable',
        volatility: stock.volatility
      };
    });
  };

  const [stocks, setStocks] = useState<StockItem[]>(generateInitialStocks());
  const [portfolio, setPortfolio] = useState<StockItem[]>([]);
  const [balance, setBalance] = useState(10000); // Starting with ₹10,000
  const [swipeCount, setSwipeCount] = useState(0);
  const maxSwipes = 15; // Increased from 10 to 15 swipes
  ...
  const simulateMarket = () => {
    ...
    const updatedPortfolio = portfolio.map(stock => {
      ...
      const priceChange = stock.currentPrice * (
        ...
        (stock.trend === 'up' ? 0.05 : 
         stock.trend === 'down' ? -0.05 : 
         stock.trend === 'volatile' ? (Math.random() > 0.5 ? 0.1 : -0.1) : 0)
      );
    ...
    const portfolioValue = updatedPortfolio.reduce((sum, stock) => sum + stock.currentPrice, 0);
    const totalValue = portfolioValue + balance;
    
    showFinalScore(totalValue, portfolioValue, updatedPortfolio);
  };

  const showFinalScore = (totalValue: number, portfolioValue: number, finalPortfolio: StockItem[]) => {
    ...
    if (profitPercentage >= 15) {
      ...
      message = `🏆 MARKET GENIUS!\nYou made a ${profitPercentage.toFixed(2)}% return!`;
      ...
      message = `📉 MARKET SLUMP!\nYou lost ${Math.abs(profitPercentage).toFixed(2)}% of your investment.`;
    } else {
    ...
    if (finalPortfolio.length > 0) {
      ...
      finalPortfolio.forEach(stock => {
        portfolioAnalysis += `• ${stock.ticker}: ₹${stock.currentPrice.toFixed(2)}\n`;
      });
    } else {
    ...
    <SafeAreaView style={styles.container}>
      <GestureHandlerRootView style={styles.gestureContainer}>
        ...
        <View style={styles.portfolioSummary}>
          <Text style={styles.portfolioLabel}>Portfolio: {portfolio.length} stocks</Text>
          <Text style={styles.portfolioValue}>
            Value: ₹{portfolio.reduce((sum, stock) => sum + stock.currentPrice, 0).toFixed(2)}
          </Text>
        </View>
        ...
        <View style={styles.cardsContainer}>
          {stocks.length > 0 ? (
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubText}>Simulating market conditions...</Text>
              <TouchableOpacity 
