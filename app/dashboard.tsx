...
import { Activity, Award, Zap, DollarSign, TrendingDown, Gamepad, Medal } from "lucide-react";
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.liveIndicatorContainer}>
        <Animated.View style={[styles.liveIndicator, { transform: [{ scale: pulseScale }] }]} />
        <Text style={styles.liveText}>Live updates - Last change: {lastUpdate.toLocaleTimeString()}</Text>
      </View>
      ...
      <View style={styles.statsGrid}>
        ...
        <StatCard 
          title="Money Made" 
          value={`$${stats.moneyMade.toLocaleString(undefined, {maximumFractionDigits: 2, minimumFractionDigits: 2})}`} 
          color="#15803d"
        />
        <StatCard 
          title="Money Lost" 
          value={`$${stats.moneyLost.toLocaleString(undefined, {maximumFractionDigits: 2, minimumFractionDigits: 2})}`} 
          color="#b91c1c"
        />
      </View>
      ...
      <View style={styles.expCard}>
        <View style={styles.expHeader}>
          <View>
            ...
            <Text style={styles.expSubtitle}>Level {stats.level} → Level {stats.level + 1}</Text>
          </View>
      ...
      <View style={styles.achievementCard}>
        ...
        <View style={styles.achievementGrid}>
          <AchievementCard 
            ...
            progress={stats.gamesPlayed > 160 ? 100 : Math.floor((stats.gamesPlayed - 156) / 5 * 100)} 
          />
          <AchievementCard 
            title="Big Spender" 
            description="Make a purchase of $1000 or more" 
            progress={stats.moneyMade > 3845.75 ? 100 : Math.floor((stats.moneyMade - 2845.75) / 10)} 
          />
          <AchievementCard 
            title="Module Master" 
            description="Complete all modules in a category" 
            progress={stats.modulesCompleted > 50 ? 100 : Math.floor((stats.modulesCompleted - 42) / 8 * 100)} 
          />
        </View>
      </View>
    </ScrollView>
...
function AchievementCard({ title, description, progress }: AchievementCardProps) {
  ...
  return (
    <View style={styles.achievementItem}>
      ...
      <View style={styles.achievementProgressBg}>
        <View 
          style={[styles.achievementProgressBar, { width: `${cappedProgress}%` }]}
        />
      </View>
